...
mod basic {
    ...
    fn test_bit_operations() {
            ...
            F: Fn(u8, u8) -> u8,
        {
            ...
            let padded_str1 = format!("{:0<longer_string_length$}", str1);
            let padded_str2 = format!("{:0<longer_string_length$}", str2);
        ...
        assert_eq!(con.bit_rdiff(result, &keys), Ok(1));
        let rdiff_result: Vec<u8> = redis::Commands::get(&mut con, result).unwrap();
        assert_eq!(rdiff_result, vec![0xF0]); // 11110000
        assert!(con.bit_rdiff(result, keys[0]).is_err());
