...
static TOKEN: Lazy<String> =
    Lazy::new(|| format!("{TOKEN_HEADER}.{TOKEN_PAYLOAD}.{TOKEN_SIGNATURE}"));
static TOKEN_WITH_NO_OID: Lazy<String> =
    Lazy::new(|| format!("{TOKEN_HEADER}.{TOKEN_PAYLOAD_NO_OID}.{TOKEN_SIGNATURE}"));
static INVALID_TOKEN: Lazy<String> = Lazy::new(|| format!("{TOKEN_HEADER}.{TOKEN_PAYLOAD}"));
...
mod entra_id_mock_tests {
    ...
    static MOCKED_TOKEN: Lazy<String> =
        Lazy::new(|| format!("mock_jwt_token.{}.{}", TOKEN_PAYLOAD, TOKEN_SIGNATURE));

    static MOCKED_TOKEN_1: Lazy<String> =
        Lazy::new(|| format!("mock_jwt_token1.{}.{}", TOKEN_PAYLOAD, TOKEN_SIGNATURE));
    static MOCKED_TOKEN_2: Lazy<String> =
        Lazy::new(|| format!("mock_jwt_token2.{}.{}", TOKEN_PAYLOAD, TOKEN_SIGNATURE));
    static MOCKED_TOKEN_3: Lazy<String> =
        Lazy::new(|| format!("mock_jwt_token3.{}.{}", TOKEN_PAYLOAD, TOKEN_SIGNATURE));
    ...
    async fn test_mock_successful_authentication() {
        ...
        let mut provider = create_mock_entra_id_credentials_provider(
            mock_credential,
            vec!["https://redis.azure.com/.default".to_string()],
        );
