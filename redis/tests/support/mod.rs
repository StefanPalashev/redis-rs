...
impl TestContext {
    ...
    pub fn new_with_cert_auth_field(tls_files: TlsFilePaths, cert_field: &str) -> TestContext {
        ...
        // Convert the address to TcpTls if needed for cert-based authentication
        let addr = match addr {
            ...
            other => other, // Already TcpTls or Unix
        };
...
/// Macro to skip tests on an existing context if the Redis version doesn't meet requirements.
/// Works with any context that has a `get_version()` method.
///
/// # Example
/// ```
/// let ctx = create_my_test_context();
/// skip_if_not_version_supported!(&ctx, &REDIS_VERSION_CE_8_6);
/// // Test continues only if version >= 8.6
/// ```
#[macro_export]
macro_rules! skip_if_not_version_supported {
