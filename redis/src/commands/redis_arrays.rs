...
impl ToRedisArgs for ArrayBound {
    ...
    {
        match self {
            Self::Start => out.write_arg(b"-"),
            Self::End => out.write_arg(b"+"),
            Self::Index(index) => index.write_redis_args(out),
        }
    }
}
...
///         .set_limit(NonZeroUsize::new(10).unwrap())
...
impl ArrayGrepOptions {
    ...
    /// Cap the number of matches returned (`LIMIT`).
    ///
    /// The server requires a positive limit, which [`NonZeroUsize`] enforces at compile time.
    pub fn set_limit(mut self, limit: NonZeroUsize) -> Self {
        self.limit = Some(limit);
