...
implement_commands! {
    ...
    fn get_ex<K: ToRedisArgs>(key: K, expire_at: Expiry) {
        let (option, time_arg) = get_expiry_option_and_time(expire_at);
    ...
    fn hget_ex<K: ToRedisArgs, F: ToRedisArgs>(key: K, fields: F, expire_at: Expiry) {
        let (option, time) = get_expiry_option_and_time(expire_at);

        cmd("HGETEX").arg(key).arg(option).arg(time).arg("FIELDS").arg(fields.num_of_args()).arg(fields)
    }
...
fn get_expiry_option_and_time(expiry: Expiry) -> (&'static str, Option<u64>) {
    match expiry {
