        ...
        F: Fn(&AccessToken) -> std::time::Duration + Send + Sync + 'static,
    {
        ...
        self.background_handle = Some(tokio::spawn(async move {
            ...
            loop {
                ...
                println!("Sleeping for {:?}", next_sleep_duration);
...
impl EntraIdCredentialsProvider<DefaultFlow> {
    ...
    pub fn start(&mut self, token_refresh_config: TokenRefreshConfig) {
        ...
        //     let scopes: Vec<&str> = scopes.iter().map(|s| s.as_str()).collect();
        //     let mut username = "default".to_string();
        //     let mut next_sleep_duration = std::time::Duration::ZERO;
        //     let mut attempt = 0;
        //     let mut error_delay = token_refresh_config.retry_config.initial_delay;
        ...
        //                 Ok(oid) => oid,
        //                 Err(e) => {
        //                     eprintln!("Failed to extract oid: {}", e);
        //                     "default".to_string()
        //                 }
        //             };
        ...
        //         } else {
        //             attempt += 1;
        //             if attempt <= token_refresh_config.retry_config.max_attempts {
        //                 error_delay = credentials_management_utils::calculate_next_delay(
        //                     error_delay,
        //                     token_refresh_config.retry_config.backoff_multiplier,
        //                     token_refresh_config.retry_config.max_delay,
        //                 );
        //                 println!("An error occurred while refreshing token. Attempt {attempt}. Sleeping for {:?}", error_delay);
        //                 tokio::time::sleep(error_delay).await;
        //                 continue;
        //             }
        //             println!("Max attempts reached. Stopping token refresh.");
        ...
        //             let response = match token_response {
        //                 Ok(access_token) => {
        //                     Ok(Self::convert_credentials(username.clone(), access_token))
        //                 }
        //                 Err(error) => Err(Self::convert_error_ref(error)),
        //             };
        ...
        //         println!("Sleeping for {:?}", next_sleep_duration);
        //         println!("Modifying to 3 sec");
        //         next_sleep_duration = std::time::Duration::from_secs(3);
...
impl EntraIdCredentialsProvider<ClientSecretFlow> {
    ...
    pub fn start(&mut self, retry_config: RetryConfig) {
        ...
        //             next_sleep_duration = remaining_duration
        //                 .checked_sub(std::time::Duration::from_secs(240))
        //                 .unwrap_or_else(|| {
        //                     eprintln!("Token expires soon; refreshing immediately");
        //                     std::time::Duration::from_secs(0)
